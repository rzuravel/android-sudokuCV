...
class SudokuBoxFragment(inRow: Int, inColumn: Int, inSubregion: Int) : Fragment() {
    ...
    private fun createValueSelectDialog() {
        ...
        viewSelectFragment.show(parentFragmentManager, VALUE_SELECT_INT)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == targetRequestCode) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (data.extras?.containsKey(VALUE_SELECT_INT)!!) {
                    val incomingValue = data.extras?.getInt(VALUE_SELECT_INT)
